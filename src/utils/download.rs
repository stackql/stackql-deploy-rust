...
use crate::utils::display::{print_info, print_error};
...
pub fn download_binary() -> Result<PathBuf, AppError> {
    ...
    let archive_name = Path::new(&download_url).file_name()
    ...
    let mut stream = response.bytes().map_err(|e| AppError::CommandFailed(format!("Failed to read response: {}", e)))?;
    ...
    if get_platform() != Platform::Windows {
        Command::new("chmod")
            ...
            .arg(&binary_path)
            .output()
            .map_err(|e| AppError::CommandFailed(format!("Failed to set executable permission: {}", e)))?;
    }
    
    print_info(&format!("StackQL executable successfully installed at: {}", binary_path.display()));
    Ok(binary_path)
}

fn extract_binary(archive_path: &Path, dest_dir: &Path, binary_name: &str) -> Result<PathBuf, AppError> {
    ...
    match get_platform() {
        Platform::MacOS => {
            ...
            let extracted_binary = unpacked_dir.join("payload").join("usr").join("local").join("bin").join("stackql");
    ...
    if !binary_path.exists() {
        return Err(AppError::CommandFailed(format!("Binary {} not found after extraction", binary_name)));
    }
